...
export function ArticlesView({ state, navigate }: ViewProps & { navigate: (view: View) => void }) {
  ...
  return <>
    ...
    {articles.length ? <div className="grid gap-4">{articles.map(article => <Card key={article.id} className="py-6.25 px-7 shadow-none gap-3 min-w-0 [&_h2]:font-serif [&_h2]:text-[22px] [&_h2]:font-medium [&_h2]:leading-[1.6] [&_h2]:wrap-anywhere [&_h2_a:hover]:text-primary [&_h2_svg]:inline [&_h2_svg]:ml-2 [&_h2_svg]:text-[#a89a80] [&_h2_svg]:align-middle max-[640px]:py-5.25 max-[640px]:px-5 max-[640px]:[&_h2]:text-[21px]"><div className="flex flex-wrap items-center gap-2.25 text-muted-foreground text-[10px] [&_[data-slot=badge]]:whitespace-normal [&_[data-slot=badge]]:wrap-anywhere"><Badge variant="secondary">{article.feedTitle}</Badge><time dateTime={article.publishedAt}>{formatDate(article.publishedAt, true)}</time>{article.dateEstimated && <Badge variant="outline" title="来源未提供有效发布时间，使用首次发现时间，不代表当日发布。">估计日期</Badge>}</div><h2>{safeUrl(article.url) ? <a href={safeUrl(article.url)} target="_blank" rel="noopener noreferrer">{article.title}<ArrowUpRight size={17} /></a> : article.title}</h2><p className="line-clamp-3 overflow-hidden text-[#777165] text-[12px] wrap-anywhere max-[640px]:text-[11px]">{article.content || '此来源未提供正文或摘要，请打开原文阅读。'}</p><div className="flex justify-between flex-wrap gap-3 pt-1.25 text-muted-foreground text-[9px]"><span>订阅提供的正文 / 摘要，非另行抓取全文</span>{safeUrl(article.url) && <a className="text-primary inline-flex items-center gap-1.25 no-underline bg-transparent border-0 text-[12px] hover:underline hover:underline-offset-3" href={safeUrl(article.url)} target="_blank" rel="noopener noreferrer">阅读原文 <ArrowUpRight size={13} /></a>}</div></Card>)}</div> : <div className={ui.emptyState}><BookOpen size={32} strokeWidth={1.3} /><h2>{query || selectedFeed ? '这一页，暂时没有匹配的文章。' : '还没有文章，先带几个来源进来。'}</h2><p>{query || selectedFeed ? '换个关键词或清除筛选。搜索范围仅为最近 500 篇缓存文章。' : '添加订阅时会获取文章；已有来源可在订阅页手动刷新。'}</p>{query || selectedFeed ? <Button variant="outline" onClick={() => { setQuery(''); setFeedId('') }}>清除筛选</Button> : <Button onClick={() => navigate('feeds')}>前往订阅源</Button>}</div>}
  </>
}
