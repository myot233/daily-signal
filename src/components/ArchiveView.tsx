  ...
  return (
    <>
      <div className={ui.viewHeading}>
        <div>
          <div className={ui.eyebrow}>阅读留痕 · 每一版都值得保存</div>
          <h1>把今天，留给未来。</h1>
          <p>按生成时间倒序保存。模板与订阅的变化，不会改写已经归档的日报。</p>
        </div>
        <Badge variant="outline">{digests.length} 份归档</Badge>
      </div>
      {selected ? (
        <>
        ...
        <div className="grid gap-3.25">
          {digests.map((digest) => (
            <Card
              ...
              key={digest.id}
            >
              <div className="w-18.75 shrink-0 flex flex-col items-center pr-5.5 border-r border-r-border [&_strong]:font-editorial [&_strong]:text-[#9b6b49] [&_strong]:font-normal [&_strong]:text-[32px] [&_strong]:leading-[1.3] [&_span]:text-[#968976] [&_span]:text-[9px] [&_span]:whitespace-nowrap max-[640px]:w-14 max-[640px]:pr-3.5 max-[640px]:[&_strong]:text-[28px] max-[640px]:[&_span]:text-[8px]">
                <strong>{digest.date.slice(8)}</strong>
                <span>{digest.date.slice(0, 7).replace('-', ' / ')}</span>
              </div>
              <button
                className="flex-1 text-left min-w-0 bg-transparent border-0 [&_h2]:font-serif [&_h2]:font-medium [&_h2]:text-[22px] [&_h2]:wrap-anywhere [&_h2_svg]:inline [&_h2_svg]:ml-2.25 [&_h2_svg]:text-[#a2957a] [&:hover_h2]:text-primary [&_p]:text-[11px] [&_p]:text-[#837a69] [&_p]:mt-1.5 [&_p]:mx-0 [&_p]:mb-0.75 [&_p]:wrap-anywhere [&_>_span]:text-[10px] [&_>_span]:text-[#9d9381] max-[640px]:[&_h2]:text-[19px] max-[640px]:[&_h2_svg]:w-3.25 max-[640px]:[&_h2_svg]:ml-1 max-[640px]:[&_p]:text-[10px] max-[640px]:[&_>_span]:text-[9px]"
        ...
        <div className={ui.emptyState}>
          ...
          <h3>时间会留下值得回看的东西。</h3>
          <p>完整生成的日报会自动归档在这里。同一天可以保留多个版本。</p>
          <Button onClick={() => navigate('today')}>去生成第一份日报</Button>
        </div>
      )}
      <Dialog
        open={!!deleting}
