...
export function TodayView({
  ...
  const templateLabel =
    state.settings.template === state.defaultTemplate ? '默认模板' : '自定义模板';
  ...
  return (
    <>
      ...
      <div className="mb-6.25 grid grid-cols-3 overflow-hidden rounded-xl border border-border bg-paper shadow-[0_2px_8px_#4b371204] max-[640px]:mb-5 max-[640px]:grid-cols-1">
        <div className="flex min-w-0 items-center gap-3.5 border-r border-border px-5 py-4 max-[800px]:gap-2.5 max-[800px]:px-3.5 max-[640px]:border-b max-[640px]:border-r-0 max-[640px]:px-4.5">
          ...
          <span className="min-w-0 text-[10px] text-muted-foreground">
            知识来源
            <strong className="mt-0.5 block text-[18px] font-semibold leading-tight text-foreground">
              {state.feeds.length}
              <small className="ml-1 font-sans text-[10px] font-normal text-muted-foreground">
                个订阅
              </small>
            </strong>
          </span>
        ...
        <div className="flex min-w-0 items-center gap-3.5 border-r border-border px-5 py-4 max-[800px]:gap-2.5 max-[800px]:px-3.5 max-[640px]:border-b max-[640px]:border-r-0 max-[640px]:px-4.5">
          ...
          <span className="min-w-0 text-[10px] text-muted-foreground">
            ...
            <strong className="mt-0.5 block text-[18px] font-semibold leading-tight text-foreground">
              ...
              <small className="ml-1 font-sans text-[10px] font-normal text-muted-foreground">
                篇文章{state.articles.length === 500 ? '（窗口内）' : ''}
              </small>
            </strong>
          </span>
        </div>
      ...
      <div className="grid items-start gap-6.25 min-[1280px]:grid-cols-[minmax(0,1fr)_21.5rem]">
        <aside
          className="min-w-0 min-[1280px]:sticky min-[1280px]:top-5 min-[1280px]:col-start-2 min-[1280px]:row-start-1"
          aria-label="日报生成选项"
        >
          <Card
            className="gap-0 overflow-hidden border-[#d8d1c3] py-0 shadow-[0_10px_30px_#4b37120a]"
          ...
          >
            <form onSubmit={generate}>
              <div className="flex items-start justify-between gap-3 border-b border-border bg-[#fbf8f1] px-5 py-4.5 max-[640px]:px-4.5">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <h2 className="font-serif text-[19px] font-semibold leading-tight">
                      AI 日报生成器
                    </h2>
                ...
                <Badge className="shrink-0" variant={generating ? 'outline' : 'secondary'}>
                  {generating ? (
                    ...
                    <span aria-hidden="true" className="size-1.5 rounded-full bg-[#6f8357]" />
                  )}
                  {generating ? '生成中' : '等待指令'}
                </Badge>
              </div>
              ...
              <div className="grid gap-4.5 px-5 py-5 max-[640px]:px-4.5 max-[640px]:py-4.5">
                <div className="grid gap-4 min-[700px]:grid-cols-2 min-[1280px]:grid-cols-1">
                  <div className="grid min-w-0 gap-2 [&_[data-slot=label]]:text-[12px]">
                    ...
                    <Input
                      ...
                      disabled={!!busy}
                      aria-describedby="digest-date-help"
                    />
                    <span id="digest-date-help" className="text-[10px] text-muted-foreground">
                      按本地时区 {timezone} 读取当天缓存。
                    </span>
                  </div>
                  <div className="grid min-w-0 gap-2 [&_[data-slot=label]]:text-[12px]">
                    ...
                    <select
                      id="digest-model"
                      className="h-10 min-w-0 rounded-md border border-input bg-paper px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
                      value={activeProviderModelId}
                      disabled={!!busy}
                      onChange={(event) => setProviderModelId(event.target.value)}
                      aria-describedby="digest-model-help"
                    >
                    ...
                    <span id="digest-model-help" className="text-[10px] text-muted-foreground">
                      生成开始时冻结选择。
                    </span>
                  </div>
                </div>
                ...
                <div
                  className="grid gap-2.5 min-[900px]:grid-cols-3 min-[1280px]:grid-cols-1"
                  aria-label="本次生成上下文"
                >
                  ...
                  <div className="flex min-w-0 items-center gap-2.5 rounded-lg border border-[#e6dfd2] bg-[#f7f3ea] px-3.5 py-3">
                    {selectedModel ? (
                      <ProviderIcon presetId={selectedModel.provider.presetId} size={16} />
                    ) : (
                      <Settings2 aria-hidden="true" className="size-4 shrink-0 text-[#9b8060]" />
                    )}
                    <span className="min-w-0 text-[10px] text-muted-foreground">
                      <strong
                      ...
                      <span className="block truncate" title={selectedModelContext}>
                ...
                <Task
                  key={generation?.sessionId ?? (generating ? 'running' : 'idle')}
                  defaultOpen={generating || Boolean(generationEvents.length)}
                  className="rounded-lg border border-[#e0d7c8] bg-paper px-4.25"
                >
                  ...
                  <TaskContent className="pb-4">
                    {generation ? (
                      generationEvents.length ? (
                        generationEvents.map((item, index) => {
                          ...
                          return (
                            <TaskItem
                              key={item.id}
                              className={`flex min-h-7 items-start gap-2.5 text-[11px] ${failed ? 'text-destructive' : ''}`}
                            >
                              {current ? (
                                <LoaderCircle
                                  aria-hidden="true"
                                ...
                                <Check
                                  aria-hidden="true"
                                  className="mt-0.5 size-3.5 shrink-0 text-[#6f8357]"
                                />
                              )}
                        ...
                        <TaskItem className="flex min-h-7 items-center gap-2.5 text-[11px]">
                          <LoaderCircle
                            aria-hidden="true"
                      ...
                      <>
                        <TaskItem className="flex min-h-7 items-center gap-2.5 text-[11px]">
                          <Database
                            aria-hidden="true"
                            className="size-3.5 shrink-0 text-[#a07c57]"
                          />
                          读取 {date ? formatDate(date) : '所选日期'} 的缓存文章
                        </TaskItem>
                        <TaskItem className="flex min-h-7 items-center gap-2.5 text-[11px]">
                          <Sparkles
                            aria-hidden="true"
                    ...
                    {generating && (
                      <p
                        className="mt-3 flex items-start gap-2 rounded-md bg-[#f4eadf] px-3 py-2.5 text-[10px] leading-[1.7] text-[#795c3e]"
                        role="status"
                      >
                        <span className="mt-1 size-1.5 shrink-0 animate-pulse rounded-full bg-primary" />
                        这些状态来自本地 SQLite
                        事件队列。离开当前页面不会取消后台生成，返回后会从已保存的事件继续显示。
                      </p>
                    )}
                  </TaskContent>
                </Task>

                <p className="quiet-note text-[10px] leading-[1.8] text-muted-foreground wrap-anywhere">
                  文章与 API Key 将发送至 <strong>{providerHost ?? '所选服务商'}</strong>。单日最多
                  500 篇、30 万字符；超限会在调用前拒绝，失败不会覆盖旧版。
                </p>

                {selectedModel && !selectedModel.provider.hasCredential && (
                  <p className="flex min-h-11 items-center gap-2 rounded-md border border-[#ead9bb] bg-[#fbf4e5] px-3.5 text-[11px] text-[#7b5c2e]">
                ...
                <div className="grid gap-3 border-t border-border pt-4.5">
                  <div className="grid grid-cols-2 gap-2.25 [&_button]:w-full">
                    ...
                    >
                      <RefreshCw
                        aria-hidden="true"
                        className={busy === '刷新订阅' ? 'animate-spin' : ''}
                      />
                      刷新订阅
                    </Button>
                    <Button
                      ...
                      className="shadow-[0_5px_14px_#a8482d20]"
                    >
                      {generating ? (
                  ...
                  <button
                    type="button"
                    className="inline-flex min-h-8 items-center justify-center gap-1.5 text-[11px] font-medium text-primary hover:underline hover:underline-offset-3"
                    onClick={() => navigate('settings')}
                  >
                    管理模型连接 <ArrowRight aria-hidden="true" size={13} />
                  </button>
                </div>
              </div>
            </form>
          ...
          {(failed.length > 0 || stale.length > 0) && (
            <div className="warning mt-4 rounded-[7px] border border-[#e8d8b2] bg-[#f7efdc] px-4 py-3.5 text-[11px] leading-[1.8] text-[#826426] wrap-anywhere [&_details]:mt-1.5 [&_ul]:mx-0 [&_ul]:mt-2 [&_ul]:list-disc [&_ul]:pl-5">
              <p>
                {failed.length > 0
                  ? `${failed.length} 个来源最近获取失败，现有缓存仍可用于生成，信息可能不完整。`
                  : '部分来源超过 24 小时未更新，建议先刷新再生成。'}
              </p>
              <details>
                ...
                <ul>
                  {state.feeds
                    ...
                    .map((feed) => (
                      <li key={feed.id}>
                        <strong>{feed.title}</strong>：{feed.error || '缓存可能已过时'}；
                        {feed.lastFetchedAt
                          ? `上次成功 ${formatDate(feed.lastFetchedAt, true)}`
                          : '尚无成功获取记录'}
                      </li>
                    ))}
                </ul>
        ...
        <section
          className="min-w-0 min-[1280px]:col-start-1 min-[1280px]:row-start-1"
          aria-labelledby="ai-output-title"
        >
          <div className="mb-3.5 flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-2.5">
              <span className="grid size-7.5 shrink-0 place-items-center rounded-lg bg-[#efe3d8] text-primary">
                <Sparkles aria-hidden="true" size={14} />
              </span>
              <div>
                <div className="text-[9px] font-semibold tracking-[1.6px] text-primary">
                  DOCUMENT PREVIEW
                </div>
                <h2 id="ai-output-title" className="font-serif text-[20px] font-semibold">
                  {report ? '最新生成结果' : '等待生成'}
                </h2>
              </div>
            </div>
          ...
          {report ? (
            ...
            <div className="relative overflow-hidden rounded-xl border border-[#dcd4c6] bg-paper px-8.75 py-9 text-center shadow-[0_8px_24px_#46371008] max-[800px]:px-5.5 max-[640px]:px-4.5 max-[640px]:py-7">
              <div
                className="pointer-events-none absolute left-1/2 top-0 h-40 w-80 -translate-x-1/2 rounded-full bg-[#efe4d7] opacity-55 blur-3xl"
                aria-hidden="true"
              />
              ...
              <h3 className="relative font-serif text-[25px] font-medium tracking-[-.5px] max-[640px]:text-[22px]">
                {state.digests.length
                  ? `${date ? formatDate(date) : '所选日期'}，还没有生成版本。`
                  : '从可信上下文，生成第一份日报。'}
              </h3>
              <p className="relative mx-auto mt-3 max-w-145 text-[12px] leading-[1.9] text-muted-foreground max-[640px]:text-[11px]">
                准备好来源、模型与模板后，生成结果会连同来源快照显示在这里。
              </p>
              <div className="relative mt-7 grid grid-cols-3 gap-3 border-t border-border pt-6 max-[800px]:gap-2 max-[640px]:mt-5.5 max-[640px]:grid-cols-1 max-[640px]:pt-5">
                <button
                  className={ui.onboardingStep}
                  type="button"
                  onClick={() => navigate('feeds')}
                >
                  <span
                    className={`grid size-7.5 place-items-center rounded-full bg-[#f3efe4] font-editorial text-[12px] font-normal text-[#685b49] max-[640px]:row-span-2 max-[640px]:self-center [&.complete]:bg-[#edf0e4] [&.complete]:text-[#536541] ${state.feeds.length ? 'complete' : ''}`}
                  >
                    {state.feeds.length ? <Check aria-hidden="true" size={17} /> : '01'}
                  </span>
                  <strong>
                    添加可信来源 <ArrowRight aria-hidden="true" size={14} />
                  </strong>
                  <span>RSS / Atom 或 OPML</span>
                </button>
                <button
                  className={ui.onboardingStep}
                  type="button"
                ...
                >
                  <span
                    className={`grid size-7.5 place-items-center rounded-full bg-[#f3efe4] font-editorial text-[12px] font-normal text-[#685b49] max-[640px]:row-span-2 max-[640px]:self-center [&.complete]:bg-[#edf0e4] [&.complete]:text-[#536541] ${state.hasApiKey ? 'complete' : ''}`}
                  >
                    {state.hasApiKey ? <Check aria-hidden="true" size={17} /> : '02'}
                  </span>
                  <strong>
                    连接 AI 模型 <ArrowRight aria-hidden="true" size={14} />
                  </strong>
                  <span>密钥只保存在本地</span>
                </button>
                <button
                ...
                >
                  <span className="grid size-7.5 place-items-center rounded-full bg-[#f3efe4] font-editorial text-[12px] font-normal text-[#685b49] max-[640px]:row-span-2 max-[640px]:self-center">
                    03
                  </span>
                  <strong>
                    定义日报模板 <ArrowRight aria-hidden="true" size={14} />
                  </strong>
                  <span>告诉 AI 如何整理</span>
                </button>
              </div>
            </div>
          )}
