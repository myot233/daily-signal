...
const fieldClass =
  'grid gap-2 [&_label]:text-[12px] [&_p]:text-[11px] [&_p]:leading-[1.8] [&_p]:text-muted-foreground';
const selectClass =
  'h-10 w-full rounded-md border border-input bg-paper px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:opacity-50';
...
function sameDraft(provider: ProviderConnection, draft: ConnectionDraft): boolean {
  return (
    ...
    JSON.stringify(provider.options) === JSON.stringify(draft.options) &&
    draft.credential === ''
  );
}
...
export function SettingsView({ state, busy, perform, notify }: ViewProps) {
  ...
  const draftProvider = draftState
    ? state.providers.find((provider) => provider.id === draftState.id)
    : undefined;
  const selected = providerId
    ? state.providers.find((provider) => provider.id === providerId)
    : (draftProvider ?? fallback);
  const draft =
    selected && draftState?.id === selected.id
      ? draftState.value
      : selected
        ? draftFor(selected)
        : null;
  ...
  const [discoveryState, setDiscoveryState] = useState<{
    providerId: string;
    models: Array<{ modelId: string; displayName: string | null }>;
  } | null>(null);
  ...
  const discoveries =
    selected && discoveryState?.providerId === selected.id ? discoveryState.models : [];
  const manualModel =
    selected && manualState?.providerId === selected.id ? manualState.modelId : '';
  ...
  const defaultChoice = availableModels.find(
    (item) => item.model.id === state.defaultProviderModelId,
  );
  ...
  function updateDraft(changes: Partial<ConnectionDraft>) {
    ...
    setDraftState({
      id: selected.id,
      revision: draftState?.id === selected.id ? draftState.revision : selected.revision,
      value: { ...draft, ...changes },
    });
  }
      ...
      async () => {
        await rpc.providers.update(
          providerUpdateSchema.parse({
            ...
            credential: draft.credential === '' ? undefined : draft.credential,
          }),
        );
        setDraftState(null);
      },
  ...
  function setDefault(providerModelId: string | null) {
    void perform(
      '切换默认模型',
      async () => {
        await rpc.defaultModel.set({ providerModelId });
      },
      providerModelId ? '日报默认模型已更新，下次生成生效。' : '日报默认模型已清空。',
    );
  }
  function saveModel(
    modelId: string,
    ...
    void perform(
      '保存模型',
      async () => {
        await rpc.providerModels.save(
          providerModelSaveSchema.parse({
  ...
  function discover() {
    ...
    void perform('获取模型目录', async () => {
      ...
      setDiscoveryState({
        providerId: selected.id,
        models: differenceBy(result.models, selected.models, (model) => model.modelId),
      });
      notify({
        kind: 'success',
        message: `找到 ${result.models.length} 个模型候选；选择后才会保存。`,
      });
    });
  }
    ...
    <>
      ...
      <Card className="mb-6 flex-row items-center justify-between gap-5 p-5 shadow-none max-[700px]:items-start max-[700px]:flex-col">
        <div className="flex min-w-0 items-center gap-3.5">
          <span className="grid size-11 shrink-0 place-items-center rounded-lg bg-secondary">
            {defaultChoice ? (
              <ProviderIcon presetId={defaultChoice.provider.presetId} />
            ) : (
              <CircleAlert size={20} />
            )}
          </span>
          <div className="min-w-0">
            ...
            <span className="text-[11px] text-muted-foreground">
              {defaultChoice?.provider.name ?? '生成前需要选择连接与模型'}
            </span>
          </div>
        </div>
        <select
          className={`${selectClass} max-w-85`}
          aria-label="日报默认模型"
          value={state.defaultProviderModelId ?? ''}
          disabled={!!busy}
          onChange={(event) => setDefault(event.target.value || null)}
        >
      ...
      <div className="grid grid-cols-[260px_minmax(0,1fr)] items-start gap-6 max-[900px]:grid-cols-1">
        <aside
          ...
          aria-label="连接列表"
        >
          <div className="px-2 py-2 text-[10px] font-semibold tracking-[.14em] text-muted-foreground">
            连接 · {state.providers.length}
          </div>
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <strong className="block truncate text-[13px]">{provider.name}</strong>
                    <span className="mt-0.5 flex gap-1.5 text-[10px] text-muted-foreground">
          ...
          {!state.providers.length && (
            <div className="px-3 py-8 text-center text-xs leading-7 text-muted-foreground">
              还没有连接。添加服务商后，可保存 Key 与模型。
            </div>
          )}
          {state.providers.length > 0 && !visibleProviders.length && (
        ...
        {selected && draft ? (
          <section className={`min-w-0 ${!providerId ? 'max-[900px]:hidden' : ''}`}>
            <div className="mb-4 flex items-center justify-between gap-4 max-[640px]:items-start">
              ...
              <Badge variant={dirty ? 'outline' : selected.enabled ? 'secondary' : 'outline'}>
                {dirty ? '有未保存修改' : selected.enabled ? '已启用' : '已停用'}
              </Badge>
            </div>
            <Tabs defaultValue="connection">
                  ...
                  >
                    <div className="grid grid-cols-2 gap-5 max-[640px]:grid-cols-1">
                      <div className={fieldClass}>
                        <Label htmlFor="provider-name">连接名称</Label>
                        <Input
                          id="provider-name"
                          required
                          maxLength={100}
                      ...
                      <div className={fieldClass}>
                        <Label htmlFor="provider-protocol">实际对外协议</Label>
                        <select
                          ...
                          disabled={
                            ...
                            (selected.presetId !== 'custom' &&
                              providerCatalog.find((item) => item.id === selected.presetId)
                                ?.protocols.length === 1)
                          }
                          onChange={(event) =>
                            updateDraft({
                              protocol: event.target.value as ConnectionDraft['protocol'],
                            })
                          }
                        >
                          {(selected.presetId === 'custom'
                            ...
                            : (providerCatalog
                                .find((item) => item.id === selected.presetId)
                                ?.protocols.map(
                                  (protocol) => [protocol, protocolLabels[protocol]] as const,
                                ) ?? [])
                          ).map(([value, label]) => (
                    ...
                    <div className={fieldClass}>
                      ...
                      <Input
                        ...
                        onChange={(event) => updateDraft({ credential: event.target.value })}
                      />
                      <p>
                      ...
                      <div className="mt-5 grid grid-cols-2 gap-5 max-[640px]:grid-cols-1">
                        <div className={fieldClass}>
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              updateOptions({ timeoutMs: Number(event.target.value) })
                            }
                          />
                        </div>
                        <div className={fieldClass}>
                          <Label htmlFor="provider-output">输出上限（tokens）</Label>
                          <Input
                            id="provider-output"
                            type="number"
                            min={32}
                            max={65536}
                            disabled={!!busy}
                            value={draft.options.maxOutputTokens}
                            onChange={(event) =>
                              updateOptions({ maxOutputTokens: Number(event.target.value) })
                            }
                          />
                        </div>
                        {(draft.protocol === 'openai-responses' ||
                          ...
                          <div className={fieldClass}>
                            <Label htmlFor="provider-reasoning">Reasoning effort</Label>
                            <select
                              id="provider-reasoning"
                              className={selectClass}
                              disabled={!!busy}
                              value={draft.options.reasoningEffort ?? ''}
                              onChange={(event) =>
                                updateOptions({
                                  reasoningEffort: event.target.value
                                    ? (event.target.value as ProviderOptions['reasoningEffort'])
                                    : undefined,
                                })
                              }
                            >
                        ...
                        {selected.presetId === 'deepseek' && (
                          <div className={fieldClass}>
                            <Label htmlFor="provider-deepseek">DeepSeek 思考</Label>
                            <select
                              id="provider-deepseek"
                              className={selectClass}
                              disabled={!!busy}
                              value={draft.options.deepseekThinking}
                              onChange={(event) => {
                                const enabled = event.target.value === 'enabled';
                                updateOptions({
                                  deepseekThinking: enabled ? 'enabled' : 'disabled',
                                  maxOutputTokens: enabled
                                    ? Math.max(16_384, draft.options.maxOutputTokens)
                                    : draft.options.maxOutputTokens,
                                });
                              }}
                            >
                            ...
                            <p>开启时自动把输出上限提高到至少 16,384。</p>
                          </div>
                        )}
                        {draft.protocol === 'anthropic-messages' && (
                          <div className={fieldClass}>
                            <Label htmlFor="provider-anthropic-thinking">Anthropic 思考预算</Label>
                            <Input
                              id="provider-anthropic-thinking"
                              type="number"
                              min={1024}
                              max={64000}
                              disabled={!!busy}
                              placeholder="留空关闭"
                              value={draft.options.anthropicThinkingBudget ?? ''}
                              onChange={(event) =>
                                updateOptions({
                                  anthropicThinkingBudget: event.target.value
                                    ? Number(event.target.value)
                                    : undefined,
                                })
                              }
                            />
                            <p>计入输出总上限，并至少为回答保留 32 tokens。</p>
                          </div>
                        )}
                        {draft.protocol === 'gemini-generative-language' && (
                          <div className={fieldClass}>
                            <Label htmlFor="provider-gemini-thinking">Gemini 思考预算</Label>
                            <Input
                              id="provider-gemini-thinking"
                              type="number"
                              min={0}
                              max={65536}
                              disabled={!!busy}
                              placeholder="留空使用模型默认"
                              value={draft.options.geminiThinkingBudget ?? ''}
                              onChange={(event) =>
                                updateOptions({
                                  geminiThinkingBudget: event.target.value
                                    ? Number(event.target.value)
                                    : undefined,
                                })
                              }
              ...
              <TabsContent value="models">
                <Card className="gap-6 p-6 shadow-none max-[640px]:p-4.5">
                  ...
                  <form
                    className="grid grid-cols-[minmax(0,1fr)_minmax(0,.7fr)_auto] gap-2 max-[640px]:grid-cols-1"
                    onSubmit={(event) => {
                      event.preventDefault();
                      saveModel(manualModel, manualName || null);
                    }}
                  >
                    <Input
                      aria-label="模型 ID"
                  ...
                  {discoveries.length > 0 && (
                    <div className="rounded-lg border bg-secondary/30 p-3">
                      <div className="mb-2 flex items-center gap-2 px-1 text-[11px] font-semibold">
                        <Search size={14} />
                        目录候选 · {discoveries.length}
                      </div>
                        ...
                        {discoveries.map((model) => (
                          ...
                          >
                            ...
                            <Button
                              ...
                              disabled={!!busy}
                              onClick={() =>
                                saveModel(model.modelId, model.displayName, 'discovered')
                              }
                            >
                              添加
                            </Button>
                        ...
                        return (
                          ...
                          >
                                ...
                                void perform(
                                  ...
                                  async () => {
                                    await rpc.providerModels.save({
                                      ...
                                      source:
                                        model.source === 'discovered' ? 'discovered' : 'manual',
                                    });
                                  },
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              disabled={!!busy || isDefault || !selected.enabled || !model.enabled}
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={
                                !!busy || !selected.hasCredential || dirty || !model.enabled
                              }
                              title={dirty ? '请先保存或放弃连接修改' : undefined}
                              onClick={() =>
                                setTestState({ providerId: selected.id, modelId: model.modelId })
                              }
                            >
                              <FlaskConical />
                              测试
                            ...
                            <Button
                              ...
                              onClick={() =>
                                void perform(
                                  '删除模型',
                                  async () => {
                                    await rpc.providerModels.remove({ id: model.id });
                                  },
                                  '模型已删除。',
                                )
                              }
      ...
      <AddProviderDialog
        ...
        busy={busy}
        perform={perform}
        onCreated={(id) => void navigate(`/settings/providers/${id}`)}
      />
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              连接、Key、模型与检测记录都会删除。若它仍是默认连接，系统会要求你先明确选择替代项或清空默认。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>发起一次真实模型测试？</DialogTitle>
            <DialogDescription>
              测试会向 {selected?.name} 发送“Reply with
              OK.”，验证地址、认证和模型调用，可能产生少量费用。检测结果会绑定当前配置版本。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
            ...
            <Button
              disabled={!!busy || !selected || !testModel}
              onClick={() => {
                if (!selected || !testModel) return;
                const modelId = testModel;
                void perform('测试模型连接', async () => {
                  const result = await rpc.providers.test({
                    providerId: selected.id,
                    modelId,
                    confirmCharge: true,
                  });
                  ...
                  notify({
                    ...
                    message: failed?.safeError ?? '连接测试通过。',
                  });
                });
              }}
            >
  ...
  return (
    <div className="rounded-lg border bg-secondary/30 p-4">
      ...
      <div className="grid grid-cols-3 gap-2 max-[640px]:grid-cols-1">
        {latest.map((check) => (
          <div key={check.id} className="rounded-md border bg-card px-3 py-2">
            <span className="text-[10px] text-muted-foreground">{label[check.stage]}</span>
            ...
            {check.safeError && (
              <p className="mt-1 text-[10px] leading-5 text-destructive">{check.safeError}</p>
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        ...
        >
          <div className="grid grid-cols-5 gap-2 max-[640px]:grid-cols-2">
          ...
          <div className="grid grid-cols-2 gap-4 max-[640px]:grid-cols-1">
            <div className={fieldClass}>
              <Label htmlFor="new-provider-name">连接名称</Label>
              <Input
            ...
            <div className={fieldClass}>
              <Label htmlFor="new-provider-protocol">实际协议</Label>
              <select
                id="new-provider-protocol"
                className={selectClass}
                value={protocol}
                disabled={presetId !== 'custom' && preset.protocols.length === 1}
                onChange={(event) =>
                  setProtocol(event.target.value as ProviderConnection['protocol'])
                }
              >
                {preset.protocols.map((value) => (
                  <option key={value} value={value}>
          ...
          <div className="grid grid-cols-2 gap-4 max-[640px]:grid-cols-1">
            <div className={fieldClass}>
              <Label htmlFor="new-provider-key">API Key（可稍后填写）</Label>
              <Input
                id="new-provider-key"
                type="password"
                autoComplete="off"
            ...
            <div className={fieldClass}>
              <Label htmlFor="new-provider-model">首个模型 ID（可选）</Label>
              <Input
                id="new-provider-model"
                className="font-mono"
                value={initialModelId}
                onChange={(event) => setInitialModelId(event.target.value)}
              />
            </div>
          </div>
          <DialogFooter>
