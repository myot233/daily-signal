...
export function Report({ digest }: { digest: Digest }) {
  function exportHtml() {
    ...
    const html = renderToStaticMarkup(
      <html lang="zh-CN"><head><meta charSet="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>{digest.title}</title><style>{exportStyles + exportLayoutStyles}</style></head>
        <body><main><header><div className="brand">DAILY SIGNAL · 技术日报</div><h1>{digest.title}</h1><p>{formatDate(digest.date)} · {digest.articleCount} 篇参考文章 · {digest.model}{digest.providerName && digest.providerProtocol ? ` · ${digest.providerName} · ${protocolLabels[digest.providerProtocol]}` : ''}</p></header>
  ...
  return <article className="max-w-210 mt-6.25 mx-auto mb-0 bg-paper border border-[#dad6c8] py-8.25 px-11.75 shadow-[0_5px_20px_#35281105] min-w-0 max-[1150px]:px-7.5 max-[640px]:py-5.5 max-[640px]:px-5 max-[640px]:mt-5">
    <div className="flex flex-wrap gap-3 justify-between items-center border-b border-b-border pb-5 mb-6.5 [&_button]:text-[10px] max-[640px]:gap-2.25 max-[640px]:mb-5.5 max-[640px]:[&_.button-row]:gap-0 max-[640px]:[&_button]:text-[9px] max-[640px]:[&_button]:px-1.75"><Badge variant="outline"><Sparkles aria-hidden="true" size={12} />AI 生成 · 已归档</Badge><div className="button-row flex items-center flex-wrap gap-1.75"><Button variant="ghost" size="sm" onClick={() => download(digest.markdown, `daily-signal-${digest.date}.md`, 'text/markdown;charset=utf-8')}><Download />Markdown</Button><Button variant="ghost" size="sm" onClick={exportHtml}><FileCode2 />导出 HTML</Button></div></div>
    <div className={ui.eyebrow}>DAILY SIGNAL / {digest.date.replaceAll('-', '.')}</div><h2 className="font-serif text-[32px] leading-[1.5] font-semibold wrap-anywhere max-[640px]:text-[27px]">{digest.title}</h2><p className="flex flex-wrap gap-2 text-muted-foreground text-[10px] mt-3.5 mx-0 mb-7.5 pb-5.75 border-b-2 border-b-[#514b3d] wrap-anywhere max-[640px]:text-[9px] max-[640px]:mb-6">{digest.articleCount} 篇参考文章 <span>·</span> {digest.model}{digest.providerName && digest.providerProtocol && <><span>·</span>{digest.providerName}<span>·</span>{protocolLabels[digest.providerProtocol]}</>} <span>·</span> {formatDate(digest.createdAt, true)}</p>
    <RichMarkdown content={digest.markdown} />
    <Sources className="mt-8 mb-0 border-t border-t-border pt-3 text-[11px] text-muted-foreground">
      <SourcesTrigger count={digest.sources.length} className="w-full font-medium hover:text-primary" />
      <SourcesContent className="mt-1 w-full gap-0">
        <ol className="list-decimal pl-5">
          {digest.sources.map(source => {
            const url = safeUrl(source.url)
            return <li className="my-1 wrap-anywhere marker:text-[#b8aa90]" key={source.id}>
              {url ? <Source className="inline-flex min-h-11 max-w-full items-center gap-1.5 text-[#87633f] hover:text-primary hover:underline hover:underline-offset-3" href={url}>
                <span className="min-w-0">{source.title}</span><ArrowUpRight aria-hidden="true" className="size-3.25 shrink-0" />
              </Source> : <span className="flex min-h-11 items-center">{source.title}</span>}
              <span className="block text-[10px] text-muted-foreground">{source.feedTitle}</span>
            </li>
          })}
        </ol>
      </SourcesContent>
    </Sources>
    <p className="mt-5 text-[10px] text-muted-foreground">由 AI 辅助整理，可能存在遗漏或误读。重要信息请点击来源核实。</p>
  </article>
}
