...
export const providerOptionsSchema = z
  ...
  .superRefine((value, ctx) => {
    if (
      value.anthropicThinkingBudget !== undefined &&
      value.anthropicThinkingBudget > value.maxOutputTokens - 32
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['anthropicThinkingBudget'],
        message: 'Anthropic 思考预算后需至少保留 32 个输出 tokens。',
      });
    }
    if (value.deepseekThinking === 'enabled' && value.maxOutputTokens < 1_024) {
      ctx.addIssue({
        ...
        message: '开启 DeepSeek 思考时，输出上限至少为 1,024。',
      });
    }
  });
...
export const providerBaseUrlSchema = z
  ...
  .transform((value, ctx) => {
    try {
      ...
      ctx.addIssue({
        code: 'custom',
        message: 'Base URL 必须是公开 HTTPS API 根地址，且不含凭据、查询参数或锚点。',
      });
      return z.NEVER;
    }
  });
...
function validateProviderFields(value: z.infer<typeof providerFieldsSchema>, ctx: z.RefinementCtx) {
  ...
  if (!preset || (preset.id !== 'custom' && !preset.protocols.includes(value.protocol))) {
    ctx.addIssue({
      code: 'custom',
      path: ['protocol'],
      message: '所选服务商不支持这个协议；自定义网关可选择任一已实现协议。',
    });
  }
  ...
  if (forbiddenSuffixes[value.protocol].test(new URL(value.baseUrl).pathname)) {
    ctx.addIssue({
      code: 'custom',
      path: ['baseUrl'],
      message: '请填写 API 根地址，不要包含协议的最终请求路径。',
    });
  }
}
...
export const credentialSchema = z
  ...
  .pipe(z.string().min(1, '请输入 API Key。'));

export const providerCreateSchema = providerFieldsSchema
  .extend({
...
export const providerModelSchema = z.object({
  ...
  options: providerModelOptionsSchema,
...
export const providerCheckSchema = z.object({
  ...
  stage: z.enum(['endpoint', 'authentication', 'model']),
...
export const providerCatalogSchema = z.array(
  z.object({
    id: z.string(),
    name: z.string(),
    ...
    protocols: z.array(providerProtocolSchema),
...
export const providerRemoveSchema = z
...
export const providerDiscoverResultSchema = z.object({
  models: z.array(providerDiscoveredModelSchema).max(500),
});
export const providerTestSchema = z
  .object({
    providerId: z.string().uuid(),
    modelId: z.string().trim().min(1).max(300),
    confirmCharge: z.literal(true),
  })
  .strict();
export const providerTestResultSchema = z.object({ checks: z.array(providerCheckSchema) });
export const providerModelSaveSchema = z
  .object({
    providerId: z.string().uuid(),
    modelId: z.string().trim().min(1, '请输入模型 ID。').max(300),
...
export const setDefaultProviderModelSchema = z
  .object({ providerModelId: z.string().uuid().nullable() })
  .strict();
