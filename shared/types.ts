  ...
  baseUrl: z
    ...
    .transform((value, ctx) => {
      try {
        ...
        ctx.addIssue({
          code: 'custom',
          message: 'Base URL 必须是 HTTPS API 根地址，不含凭据、查询参数或 /chat/completions。',
        });
        return z.NEVER;
      }
    }),
...
export const connectionSchema = modelConfigSchema
  .extend({ apiKey: apiKeySchema.optional() })
  .strict();
...
export const addFeedSchema = z
  .object({
    url: z.string().trim().min(1, '请输入订阅地址。').max(8_192),
...
>;
export type DigestGenerationSubscription = z.infer<typeof digestGenerationSubscriptionSchema>;
