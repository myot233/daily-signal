...
import { fetchPublicText } from '../network';
...
function authHeaders(protocol: ProviderProtocol, apiKey: string): Record<string, string> {
  if (protocol === 'anthropic-messages') return { 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' };
...
function upstreamError(status: number): HttpError {
  ...
  return new HttpError(502, messages[status] ?? `模型目录请求失败（HTTP ${status}）。你仍可手动添加模型。`);
}

export async function discoverModels(providerId: string, transport: ProviderTransport = fetchPublicText, signal?: AbortSignal) {
  ...
  const response = await transport(modelsEndpoint(provider.baseUrl), {
    method: 'GET', headers: authHeaders(provider.protocol, apiKey), timeoutMs: Math.min(provider.options.timeoutMs, 30_000),
    maxBytes: 512 * 1024, redirect: 'error', signal,
  });
  if (!response.ok) throw upstreamError(response.status);
  let data: unknown;
  try { data = JSON.parse(response.text); } catch { throw new HttpError(502, '模型目录返回了无法解析的 JSON。你仍可手动添加模型。'); }
  ...
  const models = rows.flatMap(row => {
    ...
    if (provider.protocol === 'gemini-generative-language' && Array.isArray(item.supportedGenerationMethods)
    ...
    const modelId = provider.protocol === 'gemini-generative-language' ? rawId.replace(/^models\//, '') : rawId;
    if (!modelId || modelId.length > 300 || /\p{Cc}/u.test(modelId)) return [];
    const rawName = typeof item.display_name === 'string' ? item.display_name : typeof item.displayName === 'string' ? item.displayName : null;
  ...
  const unique = [...byId.values()]
    .sort((left, right) => left.modelId.localeCompare(right.modelId)).slice(0, 500);
