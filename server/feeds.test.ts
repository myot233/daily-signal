...
const { db, sqlite, listArticles, listFeeds } = await import('./db');
const { articles, feeds } = await import('./schema');
const { createFeedService, exportOpml } = await import('./feeds');
after(() => sqlite.close());
...
function opml(body: string) { return `<opml version="2.0"><body>${body}</body></opml>`; }
function outline(url: string) { return `<outline type="rss" xmlUrl="${url}"/>`; }
...
test('RSS and Atom ingestion is idempotent; missing dates stay stable and can gain real dates', async () => {
  ...
  let xml = rssFixture();
  const service = createFeedService({ now: () => clock, fetchText: async url => ({ text: url.includes('atom.') ? atomFixture : xml, status: 200, ok: true }) });
  ...
  assert.deepEqual(listArticles().filter(article => article.dateEstimated).map(article => [article.id, article.publishedAt]), estimated.map(article => [article.id, article.publishedAt]));
  ...
  xml = rssFixture();
  await service.refreshFeeds();
  assert.equal(listArticles().find(article => article.id === dated.id)?.publishedAt, dated.publishedAt);
  assert.equal(listArticles().length, 6);
});
...
test('unsafe/missing article URLs are skipped; query strings distinguish articles and content is bounded', async () => {
  ...
  const service = createFeedService({ fetchText: async () => ({ text: rssFixture('', extra), status: 200, ok: true }) });
...
test('concurrent duplicate additions yield one source; invalid feeds never create rows', async () => {
  const service = createFeedService({ fetchText: async () => ({ text: rssFixture(), status: 200, ok: true }) });
  const results = await Promise.allSettled([service.addFeed(feedUrl), service.addFeed(`${feedUrl}#duplicate`)]);
  ...
  const invalid = createFeedService({ fetchText: async () => ({ text: '<html>not a feed</html>', status: 200, ok: true }) });
  ...
  await assert.rejects(service.addFeed('https://other.example.com/rss', 'x'.repeat(201)), { status: 400 });
  assert.equal(listFeeds().length, 1);
});

test('failed refresh preserves articles and last success; successful retry clears error', async () => {
  let fail = false;
  const service = createFeedService({ fetchText: async () => {
...
test('nested OPML preserves Chinese categories and escaped URLs through export/import; partial failures persist successes', async () => {
  const service = createFeedService({ fetchText: async url => {
    if (url.includes('failed.')) throw new PublicFetchError('网络请求失败。');
    return { text: rssFixture(), status: 200, ok: true };
  } });
  const input = opml(`<outline text="技术 &amp; AI"><outline title="中文">${outline('https://one.example.com/rss?a=1&amp;b=2')}${outline('https://two.example.com/rss')}${outline('https://one.example.com/rss?a=1&amp;b=2')}${outline('https://failed.example.com/rss')}</outline></outline>`);
  const result = await service.importOpml(input);
  assert.equal(result.imported, 2);
  assert.equal(result.skipped, 1);
  assert.deepEqual(result.errors.map(error => error.url), ['https://failed.example.com/rss']);
  const expected = listFeeds().map(feed => [feed.url, feed.category]).sort();
  ...
  assert.deepEqual(listFeeds().map(feed => [feed.url, feed.category]).sort(), expected);
...
test('OPML rejects DTD, invalid structure, bytes, count and depth before any network work', async () => {
  let calls = 0;
  const service = createFeedService({ fetchText: async () => { calls++; return { text: rssFixture(), status: 200, ok: true }; } });
  const cases: [string, number][] = [
    ...
    [opml(Array.from({ length: 201 }, (_, i) => outline(`https://news.example.com/${i}`)).join('')), 413],
  ...
  assert.deepEqual(listFeeds(), []);
  assert.deepEqual(await service.importOpml(opml('<outline text="x">'.repeat(20) + outline(feedUrl) + '</outline>'.repeat(20))), { imported: 1, skipped: 0, errors: [] });
});

test('DTD feed is rejected without inserting a source', async () => {
  const service = createFeedService({ fetchText: async () => ({ text: '<!DOCTYPE rss [<!ENTITY x "bad">]>' + rssFixture(), status: 200, ok: true }) });
...
test('OPML import and refresh bound simultaneous source work to four', async () => {
  ...
  const result = await service.importOpml(opml(Array.from({ length: 9 }, (_, i) => outline(`https://news.example.com/${i}`)).join('')));
...
test('invalid RSS and Atom dates become estimated instead of dropping the source', async () => {
  ...
  const service = createFeedService({
    ...
    fetchText: async url => ({
      text: url.includes('atom.') ? atomFixture.replace('<id>three</id>', '<id>three</id><published>not-a-date</published>') : rssFixture('not-a-date'),
      status: 200, ok: true,
    }),
  });
  await service.addFeed(feedUrl);
