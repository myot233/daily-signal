...
import type {
  ProviderCreateInput,
  ProviderModelSaveInput,
  ProviderUpdateInput,
} from '../../../shared/providers/schemas';
import type { ProviderCheck, ProviderConnection } from '../../../shared/providers/schemas';
...
export function listProviders(): ProviderConnection[] {
  const providerRows = db
    .select()
    .from(providers)
    .orderBy(desc(providers.updatedAt), providers.name)
    .all();
  ...
  const credentialIds = new Set(
    db
      ...
      .where(inArray(providerCredentials.providerId, ids))
      .all()
...
function validatePublicProviderUrl(value: string): string {
  ...
  if (new URL(normalized).protocol !== 'https:')
    throw new HttpError(400, '模型服务必须使用公开 HTTPS 地址。');
  ...
  sqlite.transaction(() => {
    ...
    if (input.credential)
      db.insert(providerCredentials)
        .values({ providerId: id, apiKey: input.credential, updatedAt: now })
        .run();
...
export function updateProvider(rawInput: ProviderUpdateInput) {
  ...
  if (existing.revision !== input.revision)
    throw new HttpError(409, '连接已在其他页面修改，请重新读取后再保存。');
  const defaultId = db
  ...
  if (!input.enabled && defaultId) {
    const selected = db
      .select()
      .from(providerModels)
      .where(and(eq(providerModels.id, defaultId), eq(providerModels.providerId, input.id)))
      .get();
    if (selected)
  ...
  sqlite.transaction(() => {
    const result = db
      ...
      .where(and(eq(providers.id, input.id), eq(providers.revision, input.revision)))
      .run();
    ...
    if (input.credential === null || (targetChanged && input.credential === undefined)) {
      ...
      db.insert(providerCredentials)
        .values({ providerId: input.id, apiKey: input.credential, updatedAt: now })
        .onConflictDoUpdate({
          target: providerCredentials.providerId,
          set: { apiKey: input.credential, updatedAt: now },
        })
        .run();
    }
  })();
...
export function removeProvider(id: string, revision: number) {
  ...
  if (existing.revision !== revision)
    throw new HttpError(409, '连接已在其他页面修改，请重新读取后再删除。');
  const defaultId = db
    .select({ id: settings.defaultProviderModelId })
    .from(settings)
    .where(eq(settings.id, 1))
  ...
  if (
    ...
    db
      .select()
      .from(providerModels)
      .where(and(eq(providerModels.id, defaultId), eq(providerModels.providerId, id)))
      .get()
  ) {
    throw new HttpError(409, '这是当前默认连接。请先选择其他默认模型或清空默认选择。');
  }
  const result = db
    .delete(providers)
    .where(and(eq(providers.id, id), eq(providers.revision, revision)))
    .run();
...
export function saveProviderModel(input: ProviderModelSaveInput) {
  ...
  if (existing) {
    ...
    return listProviders()
      ...
      .models.find((model) => model.id === existing.id)!;
  }
  const id = randomUUID();
  db.insert(providerModels)
    .values({
      ...
      enabled: input.enabled,
      capabilities: providerModelCapabilitiesSchema.parse({}),
      options: providerModelOptionsSchema.parse({ displayName: input.displayName || undefined }),
      source: input.source,
      createdAt: now,
      updatedAt: now,
    })
...
export function setDefaultProviderModel(id: string | null) {
  if (id) {
    ...
    if (!selected.provider.enabled || !selected.model.enabled)
      throw new HttpError(409, '停用的连接或模型不能设为默认。');
  }
...
export function replaceLegacyDefaultConnection(
  value: { baseUrl: string; model: string; deepseekThinking: 'enabled' | 'disabled' },
  credential: string | null | undefined,
) {
  const row = db.select().from(settings).where(eq(settings.id, 1)).get();
  if (!row?.defaultProviderModelId)
    throw new HttpError(409, '尚未选择默认模型，旧版设置接口不能确定要修改哪条连接。');
  const model = db
    .select()
    .from(providerModels)
  ...
  sqlite.transaction(() => {
    db.update(providers)
      .set({
        ...
        options: { ...provider.options, deepseekThinking: value.deepseekThinking },
    ...
    if (modelChanged)
      db.update(providerModels)
        .set({ modelId: value.model, updatedAt: new Date().toISOString() })
        .where(eq(providerModels.id, model.id))
        .run();
    if (credential === null || (targetChanged && credential === undefined))
      ...
      db.insert(providerCredentials)
        ...
        .onConflictDoUpdate({
          target: providerCredentials.providerId,
          set: { apiKey: credential, updatedAt: new Date().toISOString() },
        })
        .run();
  })();
}
...
export function recordProviderChecks(
  ...
  checks: Array<Pick<ProviderCheck, 'stage' | 'status' | 'latencyMs' | 'safeError'>>,
) {
