...
class ProviderStageError extends HttpError {
  constructor(
    status: number,
    message: string,
    public readonly failure: 'endpoint' | 'authentication' | 'quota' | 'model' | 'unknown',
  ) {
...
export function resolveApiKey(baseUrl: string, override?: string): string {
  ...
  if (baseUrl !== saved.baseUrl)
    throw new HttpError(400, '连接地址已修改，请填写该服务商的 API Key。');
...
async function complete(
  ...
  try {
    ...
    if (result.finishReason !== 'stop') {
      const usage =
        result.usage.outputTokens === undefined
          ? ''
          : `，已使用 ${result.usage.outputTokens} 输出 tokens`;
      const guidance =
    ...
    if (error instanceof PublicFetchError)
      throw new ProviderStageError(
        error.kind === 'timeout' || error.kind === 'cancelled' ? 504 : 502,
        error.message,
        'endpoint',
      );
...
export async function generateDigest(
  ...
  if (sources.length > 500)
    throw new HttpError(400, '当日超过 500 篇文章，为控制费用请减少订阅或选择其他日期。');
  ...
  for (const source of sources) {
    ...
    if (encoded.length + 2 > 30_000 || totalSize > 300_000) {
      throw new HttpError(
        400,
        '当日资料超过上下文保护限制（30,000 字符/篇、300,000 字符/日），请减少订阅或选择其他日期。',
      );
    }
...
export async function testSavedProviderConnection(
  ...
  if (!apiKey) {
    return {
      checks: recordProviderChecks(providerId, modelId, provider.revision, [
        { stage: 'endpoint', status: 'skipped', latencyMs: 0, safeError: null },
        {
          stage: 'authentication',
          status: 'failed',
          latencyMs: 0,
          safeError: '尚未保存 API Key。',
        },
  ...
  try {
    ...
    return {
      checks: recordProviderChecks(providerId, modelId, provider.revision, [
        {
          stage: 'endpoint',
          status: failure === 'endpoint' ? 'failed' : failure === 'unknown' ? 'skipped' : 'passed',
          latencyMs,
          safeError: failure === 'endpoint' ? message : null,
        },
        {
          stage: 'authentication',
          status: failure === 'authentication' ? 'failed' : 'skipped',
          latencyMs,
          safeError: failure === 'authentication' ? message : null,
        },
        {
          stage: 'model',
          status:
            failure === 'model' || failure === 'quota' || failure === 'unknown'
              ? 'failed'
              : 'skipped',
          latencyMs,
          safeError:
            failure === 'model' || failure === 'quota' || failure === 'unknown' ? message : null,
        },
      ]),
    };
  }
