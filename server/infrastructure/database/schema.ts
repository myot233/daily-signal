...
export const articles = sqliteTable(
  ...
  (table) => [
    ...
    index('articles_date').on(table.publishedAt),
  ],
);
export const settings = sqliteTable('settings', {
  ...
  apiKey: text('api_key'),
  defaultProviderModelId: text('default_provider_model_id').references(() => providerModels.id, {
    onDelete: 'set null',
  }),
});
...
export const providerModels = sqliteTable(
  'provider_models',
  {
    ...
    capabilities: text('capabilities', { mode: 'json' })
      ...
      .notNull(),
    options: text('options', { mode: 'json' }).$type<ProviderModelOptions>().notNull(),
    source: text('source').notNull(),
    createdAt: text('created_at').notNull(),
    updatedAt: text('updated_at').notNull(),
  },
  (table) => [
    uniqueIndex('provider_models_provider_model').on(table.providerId, table.modelId),
    index('provider_models_provider').on(table.providerId),
  ],
);
export const providerChecks = sqliteTable(
  'provider_checks',
  {
    id: text('id').primaryKey(),
    providerId: text('provider_id')
