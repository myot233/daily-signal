...
test('legacy database migrates URL, model, template, thinking and key exactly once', () => {
  ...
  try {
    ...
    const migrated = run(
      databasePath,
      `
      const { sqlite } = await import('./server/infrastructure/database/client.ts');
      const { getState } = await import('./server/modules/state/service.ts');
      const { getApiKey } = await import('./server/modules/settings/repository.ts');
