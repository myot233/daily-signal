...
export class PublicFetchError extends Error {
  constructor(
    message: string,
    public readonly kind: 'network' | 'timeout' | 'cancelled' | 'oversize' = 'network',
  ) {
    super(message);
...
export function createPublicLookup(
  resolve: ResolveAddresses = (hostname, callback) =>
    lookup(hostname, { all: true, verbatim: true }, callback),
): LookupFunction {
  ...
  return async function fetchPublicText(
    ...
    if (
      Object.keys(headers).some((key) =>
        /^(?:host|proxy-authorization|proxy-connection)$/i.test(key),
      )
    ) {
    ...
    try {
      for (let redirects = 0; ; redirects++) {
        ...
        if (response.status >= 300 && response.status < 400) {
          ...
          if (method === 'POST')
            throw new PublicFetchError('AI 服务返回重定向；为保护 API Key，请填写最终 API 地址。');
          if (options.redirect === 'error')
            throw new PublicFetchError('此请求不允许服务器重定向，请填写最终 API 地址。');
          ...
          if (new URL(next).origin !== new URL(current).origin) {
            headers = Object.fromEntries(
              Object.entries(headers).filter(
                ([key]) =>
                  !/^(?:authorization|proxy-authorization|cookie|x-api-key|x-goog-api-key)$/i.test(
                    key,
      ...
      if (options.signal?.aborted && signal.reason === options.signal.reason)
        throw new PublicFetchError('请求已取消。', 'cancelled');
      if (controller.signal.aborted)
        throw new PublicFetchError('请求超时，请稍后重试。', 'timeout');
