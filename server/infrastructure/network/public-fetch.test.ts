...
test('URL normalization rejects private/reserved destinations, credentials and non-HTTP protocols', () => {
  ...
  assert.equal(
    normalizePublicUrl(' HTTPS://Example.COM./feed?a=1#part '),
    'https://example.com/feed?a=1',
  );
  assert.equal(
    normalizePublicUrl('https://[2606:4700:4700::1111]/'),
...
test('actual connector lookup rejects mixed public/private DNS answers and never returns the private record', async () => {
  ...
  await new Promise<void>((resolve) =>
    lookup('fixture.example.com', { all: true }, (error, addresses) => {
  ...
  try {
    await assert.rejects(
      createPublicFetcher(agent)('http://fixture.example.com/'),
      (error) => error instanceof PublicFetchError && error.message.includes('私有网络'),
    );
  } finally {
    await agent.destroy();
  }
...
test('lookup honors requested families and hands the validated records directly to connector', async () => {
  ...
  const lookup = createPublicLookup((_hostname, callback) => callback(null, answers));
  await new Promise<void>((resolve) =>
    lookup('fixture.example.com', { family: 6, all: true }, (error, addresses) => {
      assert.equal(error, null);
      assert.deepEqual(addresses, [answers[1]]);
      resolve();
    }),
  );
  const privateV6 = createPublicLookup((_hostname, callback) =>
    callback(null, [{ address: '::ffff:192.168.1.1', family: 6 }]),
  );
  await new Promise<void>((resolve) =>
    privateV6('fixture.example.com', {}, (error) => {
      assert.ok(error instanceof PublicFetchError);
      resolve();
    }),
  );
...
test('GET redirects revalidate destinations; POST redirects never dispatch a second request', async () => {
  ...
  pool
    ...
    .reply(302, '', { headers: { location: '/key-leak' } });
  let leaked = false;
  pool.intercept({ path: '/key-leak', method: 'POST' }).reply(() => {
    leaked = true;
    return { statusCode: 200, data: 'unexpected' };
  });
  const fetchText = createPublicFetcher(agent);
  try {
    await assert.rejects(
      fetchText('https://fixture.example.com/private'),
      (error) => error instanceof PublicFetchError && error.message.includes('私有网络'),
    );
    await assert.rejects(
      fetchText('https://fixture.example.com/post', {
        method: 'POST',
        body: '{}',
        headers: { authorization: 'Bearer sentinel-key' },
      }),
      (error) => error instanceof PublicFetchError && error.message.includes('重定向'),
    );
...
test('GET follows four redirects but rejects the fifth', async () => {
  ...
  for (let index = 0; index < 4; index++)
    pool
      .intercept({ path: `/ok${index}` })
      .reply(302, '', { headers: { location: `/ok${index + 1}` } });
  pool.intercept({ path: '/ok4' }).reply(200, 'last');
  for (let index = 0; index < 5; index++)
    pool
      .intercept({ path: `/loop${index}` })
      .reply(302, '', { headers: { location: `/loop${index + 1}` } });
  try {
    assert.deepEqual(await createPublicFetcher(agent)('https://fixture.example.com/ok0'), {
      text: 'last',
      status: 200,
      ok: true,
    });
    await assert.rejects(
      createPublicFetcher(agent)('https://fixture.example.com/loop0'),
      (error) => error instanceof PublicFetchError && error.message.includes('重定向'),
    );
  } finally {
    await agent.close();
...
test('cross-origin GET redirects remove credentials', async () => {
  ...
  agent.disableNetConnect();
  agent
    .get('https://fixture.example.com')
    .intercept({ path: '/' })
    .reply(302, '', { headers: { location: 'https://other.example.com/' } });
  ...
  agent
    ...
    .reply(200, 'safe');
  try {
    await createPublicFetcher(agent)('https://fixture.example.com/', {
      headers: {
        authorization: 'Bearer sentinel-key',
        cookie: 'session=sentinel-key',
        'x-api-key': 'anthropic-sentinel',
        'x-goog-api-key': 'gemini-sentinel',
        'x-custom-token': 'custom-sentinel',
      },
    });
...
test('credentialed GET callers can forbid redirects before a second origin is contacted', async () => {
  ...
  agent
    .get('https://fixture.example.com')
    .intercept({ path: '/models' })
    .reply(302, '', { headers: { location: 'https://other.example.com/models' } });
  let contacted = false;
  agent
    .get('https://other.example.com')
    .intercept({ path: '/models' })
  ...
  try {
    await assert.rejects(
      createPublicFetcher(agent)('https://fixture.example.com/models', {
        headers: { 'x-goog-api-key': 'gemini-sentinel' },
        redirect: 'error',
      }),
...
test('timeout interrupts a stalled response body and closes the connection', async () => {
  ...
  try {
    await assert.rejects(
      fixture.fetchText(fixture.url, { timeoutMs: 100 }),
      (error) => error instanceof PublicFetchError && error.kind === 'timeout',
    );
    await closed.promise;
  } finally {
    await fixture.close();
...
test('already cancelled calls do not dispatch a request; HTTP failures return status without leaking body errors', async () => {
  ...
  agent
    ...
    .reply(() => {
      requested = true;
      return { statusCode: 401, data: 'upstream response' };
    });
  const fetchText = createPublicFetcher(agent);
  try {
    await assert.rejects(
      fetchText('https://fixture.example.com/', { signal: AbortSignal.abort('secret') }),
      (error) => error instanceof PublicFetchError && error.kind === 'cancelled',
    );
    assert.equal(requested, false);
    assert.deepEqual(await fetchText('https://fixture.example.com/'), {
      text: 'upstream response',
      status: 401,
      ok: false,
    });
    agent
      ...
      .replyWithError(new Error('sentinel-secret-raw-error'));
    await assert.rejects(
      fetchText('https://fixture.example.com/error'),
      (error) => error instanceof PublicFetchError && !error.message.includes('sentinel'),
    );
  } finally {
    await agent.close();
  }
